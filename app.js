...
// NOVO: Listener para Puxada Manual de Ticket
...
if (btnManualSync) {
    btnManualSync.onclick = async () => {
        ...
        if (!number) {
            showToast('Digite o número do chamado!', 'warning');
            return;
        }
        ...
        btnManualSync.innerHTML = '...';
        btnManualSync.disabled = true;

        try {
            // Reutiliza a função de fetch mas passando o parâmetro forceTicket no corpo (via globais ou alteração na assinatura)
            // Para ser mais limpo, vamos disparar o fetch customizado aqui mesmo
            const syncButton = document.getElementById('btnSyncTiFlux');
            const syncIcon = syncButton?.querySelector('.icon-sync');
            if (syncIcon) syncIcon.classList.add('rotating');
            if (syncButton) syncButton.disabled = true;

            // Busca IDs atuais para o corpo do POST
            const currentSnap = await db.collection('tasks').get();
            const openTicketsIds = [];
            currentSnap.forEach(doc => {
                const data = doc.data();
                if (data.number && (data.status && !data.status.includes('Concluida'))) {
                    openTicketsIds.push(data.number);
                }
            });
            ...
            const fetchOptions = {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    openTickets: openTicketsIds,
                    forceTicket: number 
                })
            };

            let response = await fetch('/api/demandas', fetchOptions);
            if (!response.ok) response = await fetch('/demandas', fetchOptions);
            
            if (!response.ok) throw new Error('Falha no servidor');

            const apiTasks = await response.json();
            // A lógica de processamento dos apiTasks já está no onSnapshot do Firebase
            // pois o servidor vai salvar no Firestore e o onSnapshot vai reagir.
            // Mas precisamos avisar o usuário se deu certo.
            
            const foundPushed = apiTasks.find(t => String(t.number) === String(number));
            if (foundPushed) {
                showToast(`Chamado #${number} puxado com sucesso!`, 'success');
                input.value = '';
            } else {
                showToast(`Chamado #${number} não encontrado ou não pertence a uma categoria monitorada.`, 'warning');
            }

        } catch (error) {
            console.error(error);
            showToast('Erro ao puxar chamado.', 'critical');
        } finally {
...
function openModal() {
    ...
    if (document.getElementById('taskEtapa')) document.getElementById('taskEtapa').value = '';
...
function openEditModal(id) {
    ...
    document.getElementById('taskEtapa').value = task.etapa || '';
        }
    ...
    filteredTasks.forEach(task => {
        ...
        const cardHTML = `
            <div class="task-card ${sla.class} ${isCompleted ? 'completed-card' : ''}" draggable="${!isCompleted}" data-id="${task.id}">
                ...
                <div style="font-size: 0.70rem; color: var(--text-muted); margin-top: 2px; display: flex; align-items: center; gap: 4px;">
                ...
                <div style="font-size: 0.70rem; color: var(--text-muted); margin-top: 2px; margin-bottom: 8px; display: flex; flex-wrap: wrap; gap: 10px;">
                    <div style="display: flex; align-items: center; gap: 4px;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M12 2v4a2 2 0 0 0 2 2h4"/><path d="M12 18V6a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2Z"/><path d="M2 18V6a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2Z"/>
                        </svg>
                        Estágio: <span style="color: var(--text-secondary); font-weight: 500;">${task.etapa || 'N/A'}</span>
                    </div>
                    ${task.lastDevCheck ? `
                    <div style="display: flex; align-items: center; gap: 4px;" title="Data da última verificação ou alteração no TiFlux">
                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                        </svg>
                        Última Atividade: ${formatDate(task.lastDevCheck)}
                    </div>` : ''}
                </div>
